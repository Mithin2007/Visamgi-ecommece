...
export async function buyNow(f:FormData){const user=await requireCustomer();await addCartItem(user.id,String(f.get("productId")),String(f.get("variantId")||"")||undefined,Number(f.get("quantity")));revalidatePath("/cart");redirect("/checkout");}
