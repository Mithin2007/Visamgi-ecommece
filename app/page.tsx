...
export default function HomePage() {
  return <main>
    ...
    <section className="hero" aria-labelledby="hero-title">
      ...
      <div className="hero__content shell"><p className="eyebrow">Objects with a memory</p><h1 id="hero-title">Heritage,<br />beautifully lived.</h1><p className="hero__lede">A considered collection of Indian craft, designed to bring warmth, ritual and character to the everyday.</p><Link className="button button--light" href="/collections">Explore the collection</Link></div>
    </section>
    ...
    <section className="collections shell" aria-labelledby="collections-title"><div className="section-heading"><p className="eyebrow">Discover slowly</p><h2 id="collections-title">The collection</h2><Link href="/collections">View all <span aria-hidden="true">↗</span></Link></div><div className="collection-grid">{collections.map((collection) => <Link className="collection-card" href="/collections" key={collection.name}><div className="collection-card__image"><Image src={collection.image} alt="" fill sizes="(max-width: 700px) 100vw, 33vw" /></div><h3>{collection.name}</h3><span>Explore <span aria-hidden="true">→</span></span></Link>)}</div></section>
