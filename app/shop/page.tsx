...
export default async function Shop({searchParams}:{searchParams:Promise<ShopSearch>}){
  try{
    ...
    const [products,categories]=await Promise.all([db.product.findMany({where,include:{images:{orderBy:{sortOrder:"asc"}}},orderBy}),db.category.findMany({where:{published:true},orderBy:{sortOrder:"asc"}})]);
  ...
  }catch{return <main><SiteHeader/><section className="route-state shell"><p className="eyebrow">The collection</p><h1>We’re refreshing the collection.</h1><p>Please try again in a moment.</p><Link className="button" href="/shop">Try again</Link></section><SiteFooter/></main>;}
}
