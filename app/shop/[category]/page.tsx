...
export default async function Category({params}:{params:Promise<{category:string}>}){
  ...
  const row=await db.category.findFirst({where:{slug:category,published:true},include:{products:{where:{published:true,archivedAt:null},include:{images:{orderBy:{sortOrder:"asc"}}},orderBy:{createdAt:"desc"}}}});
  if(!row)notFound();
