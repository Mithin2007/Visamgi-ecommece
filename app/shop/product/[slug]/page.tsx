...
export default async function Product({params}:{params:Promise<{slug:string}>}){
  ...
  const product=await withCatalogueFallback(`product:${slug}`,()=>db.product.findFirst({where:{slug,published:true,archivedAt:null},include:{images:{orderBy:{sortOrder:"asc"}},variants:true}}),null);
  if(!product)notFound();
  const related=await withCatalogueFallback(`related:${product.id}`,()=>db.product.findMany({where:{categoryId:product.categoryId,published:true,archivedAt:null,id:{not:product.id}},include:{images:{orderBy:{sortOrder:"asc"}}},orderBy:{createdAt:"desc"},take:3}),[]);
